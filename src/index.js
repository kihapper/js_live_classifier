...
const classes = ['bicycle', 'exit', 'fire extinguisher', 'sphere cam', 'square cam','street']
...
(async function main() {
  try {
    alert("IMPORTANT:The neural network model is about 15MB big");
    await webcam.setup();
...
document.getElementById('capture-button').addEventListener('click', async () => {
  ...
  console.log('predict...');
  const topK = mobileNet.getTopKClasses(result, 12);
